...
class LoginWindow:
    ...
    def add_frame(self):
        ...
        self.label = Label(self.frame, text="使用者登入")
        ...
        self.button1 = Button(self.frame, text="註冊",
                             font='微軟正黑體 15 bold',
    ...
    def login(self):
        ...
        if self.userid.get() == "":
            ...
            if res:
                messagebox.showinfo("系統", "登入成功!")
                self.win.destroy()
                ch = selectsong.SelectWindow()
                ch.add_frame(res[1])
            else:
                messagebox.showinfo("警告!", "錯誤的帳號或密碼")
    def register(self):
        self.win.destroy()
